...
function BrandCard({
  ...
  return (
    <motion.div
      ref={setRef}
      data-index={index}
      className="min-w-0 basis-auto md:flex-1 md:basis-0"
      // Hover-expand is a desktop-only effect. On touch every card keeps an
      // equal share so the column reads as a clean static stack.
      animate={{ flexGrow: !viewportMode && isActive ? 2.5 : 1 }}
    ...
    >
      <Link
        ...
        className="relative block h-[54vh] md:h-[72vh] overflow-hidden rounded-2xl cursor-pointer group"
      >
        {/* Image */}
        <Image
          src={brand.image}
